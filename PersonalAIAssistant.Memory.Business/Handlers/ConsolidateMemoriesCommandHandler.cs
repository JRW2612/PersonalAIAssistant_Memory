...
/// Validates that all merged memory IDs belong to the calling user.
...
public class ConsolidateMemoriesCommandHandler : IRequestHandler<ConsolidateMemoriesCommand, Guid>
{
    ...
    public ConsolidateMemoriesCommandHandler(
        ...
        IAIProviderFactory aiFactory,
        ILogger<ConsolidateMemoriesCommandHandler> logger)
        : this(eventStore, eventBus, aiFactory, null!, logger)
    {
    }

    public async Task<Guid> Handle(ConsolidateMemoriesCommand request, CancellationToken cancellationToken)
    {
        // ── 0. BOLA Defense: Verify ownership of all merged memory candidates ──
        if (_readRepo != null && request.MergedMemoryIds != null && request.MergedMemoryIds.Count > 0 &&
            !string.IsNullOrWhiteSpace(request.UserId) && request.UserId != "system")
        {
            var memories = await _readRepo.GetMemoriesByIdsAsync(request.MergedMemoryIds, cancellationToken);
            if (memories.Any(m => !string.IsNullOrEmpty(m.UserId) && !string.Equals(m.UserId, request.UserId, StringComparison.OrdinalIgnoreCase)))
            {
                _logger.LogWarning(
                    "[SECURITY VIOLATION] User {UserId} attempted unauthorized consolidation on memories owned by another user.",
                    request.UserId);
                throw new UnauthorizedAccessException("One or more memory IDs do not belong to the current user.");
            }
        }
